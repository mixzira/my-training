...
- The exact backdrop-filter blur radius on `{component.sub-nav-frosted}` and `{component.floating-sticky-bar}` is platform-dependent.

---

# Aplicação deste design system em `my-training`

> Esta seção é o **contrato de tradução** entre o documento acima e o código deste projeto.
> A fonte da verdade em código são os tokens declarados em `@theme` no arquivo [globals.css](../src/app/globals.css).

## Adaptação de domínio: "produto" → "treino"

O sistema foi analisado sobre um catálogo de produtos. Aqui, o **produto é o treino**.

| Papel no design system original | Equivalente em `my-training` |
|---|---|
| Product tile (herói de produto) | Card/tile de um treino ou de uma sessão |
| Product photography + o único drop-shadow | **Não existe fotografia.** O shadow do sistema **não é usado**. O peso vem do número (carga/reps) em display size |
| Store utility card | Card de exercício dentro do treino, e itens de listas |
| Configurator option chip | Seletor de série, de grupo muscular, de treino do dia |
| Floating sticky bar | Barra de ação fixa na sessão em andamento ("Finalizar treino") |
| Global nav (44px, preto) | Header do app |
| Sub-nav frosted | Barra de contexto da rota (nome do treino + ação primária) |

**Substituição do único shadow:** como não há fotografia de produto, o sistema fica sem nenhuma sombra. Hierarquia = mudança de superfície (claro ↔ escuro) + tipografia. Essa é a regra 7 do Iteration Guide levada a sério.

## Fonte

SF Pro é proprietária. Em macOS/iOS a stack `system-ui, -apple-system` resolve para SF Pro real; nas demais plataformas usamos **Inter** (via `next/font/google`), com os ajustes prescritos na seção "Note on Font Substitutes": `-0.01em` extra em display e `line-height` de body em 1.44.

## Mobile-first é obrigatório

A tabela de breakpoints acima é lida **de baixo para cima**. Escreva o estilo do telefone sem prefixo e só então adicione `sm:`/`md:`/`lg:`. Os breakpoints do Tailwind foram redefinidos no `@theme` para bater com os valores estruturais do documento:

| Token Tailwind | Valor | Origem |
|---|---|---|
| `sm` | 480px | small phone |
| `md` | 640px | phone |
| `lg` | 834px | tablet landscape switch |
...
6. **`active:scale-95`** em todo botão — é a micro-interação do sistema.
7. **Alvo de toque mínimo 44px** (`min-h-touch`).
8. **Tiles full-bleed não têm raio.** Cards utilitários usam `rounded-lg` (18px); ações usam `rounded-pill`.
